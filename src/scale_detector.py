...
class FloorPlanScaleDetector:
    ...
    def detect_scale_numbers(self, region: str = "top", region_ratio: float = 0.15) -> List[Dict]:
        """
        检测指定区域的比例尺数字
    ...
    def detect_arrow_lines_near_number(self, number_bbox: Tuple[int, int, int, int], 
                                       search_radius: int = 50) -> List[Tuple[int, int, int, int]]:
        """
        检测数字附近的箭头线段
    ...
    def find_arrow_line_for_number(self, number_bbox: Tuple[int, int, int, int]) -> Optional[Tuple[int, int, int, int]]:
        """
        为指定数字找到对应的双箭头线段
        ...
        for line in lines:
            ...
            # 计算线段中心到数字中心的距离
            distance = np.sqrt((line_center_x - center_x)**2 + (line_center_y - center_y)**2)
    ...
    def calculate_scale_ratio(self, target_number: Optional[int] = None, 
                            region: str = "top") -> Optional[Dict]:
        """
        计算比例尺比例
    ...
    def visualize_detections(self, save_path: Optional[str] = None, 
                           show_all: bool = False, show_arrows: bool = True) -> np.ndarray:
        """
        可视化检测结果
        ...
        for i, num_info in enumerate(numbers):
            ...
            if show_arrows:
                ...
                if arrow_line:
                    x1, y1, x2, y2 = arrow_line
                    # 用蓝色绘制箭头线段
                    cv2.line(img_vis, (int(x1), int(y1)), (int(x2), int(y2)), (255, 0, 0), 3)
                    # 添加线段长度标注
                    line_length = np.sqrt((x2 - x1)**2 + (y2 - y1)**2)
                    mid_x, mid_y = int((x1 + x2) / 2), int((y1 + y2) / 2)
                    cv2.putText(img_vis, f"{line_length:.1f}px", (mid_x, mid_y - 15), 
    ...
    try:
        ...
        if main_scale:
            ...
            if main_scale['arrow_line']:
                ...
                print(f"  未找到箭头线段，使用文本框尺寸: {main_scale['pixel_length']:.1f} 像素")
        ...
        for i, candidate in enumerate(candidates[:10]):  # 显示前10个
            print(f"  {i+1}. 数字:{candidate['number']}, "
