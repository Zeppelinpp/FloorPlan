...
class ScaleDetector:
    ...
    def detect_arrows_and_numbers(self) -> List[Tuple[int, Tuple[int, int, int, int]]]:
        ...
        ocr_data = pytesseract.image_to_data(
            self.gray, output_type=Output.DICT, config="--psm 6"
        )
        ...
        for i, text in enumerate(ocr_data["text"]):
            if text.strip():
                ...
                if (
                    cleaned_text.isdigit() and len(cleaned_text) >= 3
                ):  # 至少3位数，过滤噪声
                    x = ocr_data["left"][i]
                    y = ocr_data["top"][i]
                    w = ocr_data["width"][i]
    ...
    def find_scale_numbers_in_top_region(
        self, top_ratio: float = 0.15
    ...
    def detect_horizontal_lines_near_numbers(
        self, numbers: List[Tuple[int, Tuple[int, int, int, int]]]
    ) -> List[Tuple]:
        """
